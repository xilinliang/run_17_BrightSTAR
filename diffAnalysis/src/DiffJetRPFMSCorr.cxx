...
void DiffJetRPFMSCorr(TString inFileName, TString outName, TString mode, TString det)
{
    ...
    TH2D *h2_BBCW_vs_BBCE_signal = new TH2D("h2_BBCW_vs_BBCE_signal","small BBC west ADC vs small BBC east ADC (signal);small BBC east ADC;small BBC west ADC",200,0,20000,200,0,20000);
    TH2D *h2_BBCW_vs_BBCE_bkg = new TH2D("h2_BBCW_vs_BBCE_bkg","small BBC west ADC vs small BBC east ADC (bkg);small BBC east ADC;small BBC west ADC",200,0,20000,200,0,20000);
    ...
    TH2D *h2_RPphi_vs_FMSphi = new TH2D("h2_RPphi_vs_FMSphi","RP track phi vs FMS EM jet phi;FMS EM jet #phi[rad];RP track #phi[rad]",Phibin,-TMath::Pi(),TMath::Pi(),Phibin,-TMath::Pi(),TMath::Pi());
    TH1D *h1_B_CphiPCdeltaphi = new TH1D("h1_B_CphiPCdeltaphi","Blue beam cos(#phi_{P})*cos(#Delta_{#phi});cos(#phi_{P})*cos(#Delta_{#phi})",5,-1,1);
    TH1D *h1_Y_CphiPCdeltaphi = new TH1D("h1_Y_CphiPCdeltaphi","Yellow beam cos(#phi_{P})*cos(#Delta_{#phi});cos(#phi_{P})*cos(#Delta_{#phi})",5,-1,1);
    TH1D *h1_BU_CphiPCdeltaphi = new TH1D("h1_BU_CphiPCdeltaphi","Blue beam cos(#phi_{P})*cos(#Delta_{#phi}) for blue up spin;cos(#phi_{P})*cos(#Delta_{#phi})",7,-1.15,1.15);
    TH1D *h1_BD_CphiPCdeltaphi = new TH1D("h1_BD_CphiPCdeltaphi","Blue beam cos(#phi_{P})*cos(#Delta_{#phi}) for blue down spin;cos(#phi_{P})*cos(#Delta_{#phi})",7,-1.15,1.15);
    TH1D *h1_YU_CphiPCdeltaphi = new TH1D("h1_YU_CphiPCdeltaphi","Yellow beam cos(#phi_{P})*cos(#Delta_{#phi}) for yellow up spin;cos(#phi_{P})*cos(#Delta_{#phi})",7,-1.15,1.15);
    TH1D *h1_YD_CphiPCdeltaphi = new TH1D("h1_YD_CphiPCdeltaphi","Yellow beam cos(#phi_{P})*cos(#Delta_{#phi}) for yellow down spin;cos(#phi_{P})*cos(#Delta_{#phi})",7,-1.15,1.15);
    TH1D *h1_IP_Cphi = new TH1D("h1_IP_Cphi","cos(#phi_{EM jet}) for in plane;cos(#phi_{EM jet})",7,-1,1);
    TH1D *h1_IU_Cphi = new TH1D("h1_IU_Cphi","cos(#phi_{EM jet}) for in plane spin up;cos(#phi_{EM jet})",7,-1,1);
    TH1D *h1_ID_Cphi = new TH1D("h1_ID_Cphi","cos(#phi_{EM jet}) for in plane spin down;cos(#phi_{EM jet})",7,-1,1);
    TH1D *h1_OP_Cphi = new TH1D("h1_OP_Cphi","cos(#phi_{EM jet}) for out of plane;cos(#phi_{EM jet})",7,-1,1);
    TH1D *h1_OU_Cphi = new TH1D("h1_OU_Cphi","cos(#phi_{EM jet}) for out of plane spin up;cos(#phi_{EM jet})",7,-1,1);
    TH1D *h1_OD_Cphi = new TH1D("h1_OD_Cphi","cos(#phi_{EM jet}) for out of plane spin down;cos(#phi_{EM jet})",7,-1,1);
    TH1D *h1_DeltaPhi = new TH1D("h1_DeltaPhi","#Delta_{#phi} = #phi_{EM jet} - #phi_{RP};#Delta_{#phi}",40,-1.5,4.5);
    TH1D *h1_cosPhicosDeltaPhi = new TH1D("h1_cosPhicosDeltaPhi","cos(#phi)*cos(#Delta_{#phi})",100,-1.15,1.15);
	...
	if ((nRpsTracksEast>0)||(nRpsTracksWest==2)||(!goodevent)) {continue;}
...
//	    if (!good_emjet_pt) continue;
	...
	//if ((det=="fms")&&(mode!="largeBBC")&&(mode!="smallBBC")&&(mode!="smallBBCE")) {if ((nRpsTracksWest==1)&&((skimEvent->GetBbcSumSmall(1) > 450)||(skimEvent->GetBbcSumSmall(0) < 220))) {continue;}}
	if ((det=="fms")&&(mode=="smallBBC")) {if ((nRpsTracksWest==1)&&((skimEvent->GetBbcSumSmall(1) > 400))) {continue;}}
	//if ((det=="fms")&&(mode=="smallBBC")) {if ((nRpsTracksWest==1)&&((skimEvent->GetBbcSumSmall(1) > 400)||(skimEvent->GetBbcSumSmall(0) < 220))) {continue;}}
	if ((det=="fms")&&(mode=="smallBBCE")) {if ((nRpsTracksWest==1)&&((skimEvent->GetBbcSumSmall(1) > 450)||(skimEvent->GetBbcSumSmall(0) < 180))) {continue;}}
...
//	if ((det=="fms")&&(mode=="smallBBC")) {if ((nRpsTracksWest==1)&&((skimEvent->GetBbcSumSmall(1) > 105)||(skimEvent->GetBbcSumLarge(1) > 60))) {continue;}}
	...
	//	else {if ((eng > 10)&&(eng < 60)) {eng_corr = f2->Eval(eng);} else {eng_corr = eng;}}
	    ...
	    if ((xF > 0.3)&& (xF < 0.35)) {h1_sumEwest_XF3035->Fill(RpWestPpJetE);h1_xF_3035->Fill(xF);}
	    if ((xF > 0.35)&& (xF < 0.4)) {h1_sumEwest_XF3540->Fill(RpWestPpJetE);h1_xF_3540->Fill(xF);}
	    if ((xF > 0.4)&& (xF < 0.45)) {h1_sumEwest_XF4045->Fill(RpWestPpJetE);h1_xF_4045->Fill(xF);}
	    //if ((xF > 0.3)&&(xF < 0.45)) {h1_sumEwest_XF30->Fill(RpWestPpJetE);h1_xF_3045->Fill(xF);xF = 0.325;}
	    ...
	    if (jet->GetNumberOfTowers() != 2) continue;

	    Double_t YRPphi=0;
	    Double_t deltaphi=0;
	    if (phi > 0) {Yphi = TMath::Pi() - phi;} else {Yphi = -1.0 * TMath::Pi() - phi;}
	    if (WestRpPhi > 0) {YRPphi = TMath::Pi() - WestRpPhi;} else {YRPphi = -1.0 * TMath::Pi() - WestRpPhi;}
	    deltaphi=phi-WestRpPhi;
	    //if (deltaphi < - TMath::Pi()) {deltaphi = deltaphi + TMath::Pi();}
	    //if (deltaphi >  TMath::Pi()) {deltaphi = deltaphi - TMath::Pi();}
	    Double_t cosPcosdphi;
	    if (((deltaphi > -TMath::Pi() / 6.0)&&(deltaphi < TMath::Pi() / 6.0))||(((deltaphi > 5*TMath::Pi() / 6.0)&&(deltaphi < 7*TMath::Pi() / 6.0)))) 
	    {
		//cosPcosdphi=cos(WestRpPhi)*cos(phi-WestRpPhi);
		if (skimEvent->GetSpinB() == 1) {h1_IU_Cphi->Fill(cos(phi));}
		if (skimEvent->GetSpinB() == -1) {h1_ID_Cphi->Fill(cos(phi));}
	    }
	    if (((deltaphi > TMath::Pi() / 3.0)&&(deltaphi < 2*TMath::Pi() / 3.0))||(((deltaphi > -2*TMath::Pi() / 3.)&&(deltaphi < -TMath::Pi() / 3.0)))) 
	    {
		//cosPcosdphi=cos(WestRpPhi)*cos(phi-WestRpPhi);
		if (skimEvent->GetSpinB() == 1) {h1_OU_Cphi->Fill(cos(phi));}
		if (skimEvent->GetSpinB() == -1) {h1_OD_Cphi->Fill(cos(phi));}
		
	    }

	    h1_cosPhicosDeltaPhi->Fill(cos(WestRpPhi)*cos(phi-WestRpPhi));

	    if (skimEvent->GetSpinB() == 1) {h1_BU_CphiPCdeltaphi->Fill(cos(WestRpPhi)*cos(phi-WestRpPhi));}
	    if (skimEvent->GetSpinB() == -1) {h1_BD_CphiPCdeltaphi->Fill(cos(WestRpPhi)*cos(phi-WestRpPhi));}
	    if (skimEvent->GetSpinY() == 1) {h1_YU_CphiPCdeltaphi->Fill(cos(YRPphi)*cos(Yphi-YRPphi));}
	    if (skimEvent->GetSpinY() == -1) {h1_YD_CphiPCdeltaphi->Fill(cos(YRPphi)*cos(Yphi-YRPphi));}
	    h2_RPphi_vs_FMSphi->Fill(phi,WestRpPhi);
	    h1_DeltaPhi->Fill(WestRpPhi-phi);
	    if ((skimEvent->GetSpinB() == 1)||(skimEvent->GetSpinB() == -1)) {h1_B_CphiPCdeltaphi->Fill(cos(WestRpPhi)*cos(phi-WestRpPhi));}
	    if ((skimEvent->GetSpinY() == 1)||(skimEvent->GetSpinY() == -1)) {h1_Y_CphiPCdeltaphi->Fill(cos(YRPphi)*cos(Yphi-YRPphi));}
