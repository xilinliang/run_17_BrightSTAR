...
void DiffJetXFANCalc_4pt_bkg(TString inFileName, TString outName, TString outNameText, TString det)
{
    ...
    BlueBeamUp = (TH2D*) file->Get("h2_BU_xF_vs_phi_bkg");
    BlueBeamDown = (TH2D*) file->Get("h2_BD_xF_vs_phi_bkg");
    YellowBeamUp = (TH2D*) file->Get("h2_YU_xF_vs_phi_bkg");
    YellowBeamDown = (TH2D*) file->Get("h2_YD_xF_vs_phi_bkg");
	...
	TH2D * temp = new TH2D("temp","A_{N}",8,0.1,0.35,20,-0.06,0.10);
	...
	TLatex T2;
   	...
   	T2.DrawLatex(0.11,0.09,"p^{#uparrow} + p #rightarrow EM jet + p + X");
   	T2.DrawLatex(0.11,0.08,"#sqrt{s} = 510 GeV");
   	T2.DrawLatex(0.11,0.07,"Background region from E sum cut");
   	//T2.DrawLatex(0.11,-0.09,"3.0\% polarization scale uncertainty not shown");
	
	
	ANplot->SaveAs("FMS_ANplot_4pt_bkg.pdf");
	ANplot->SaveAs("FMS_ANplot_4pt_bkg.png");
	ANplot->SaveAs("FMS_ANplot_4pt_bkg.root");
	...
	////ANplot->SetLeftMargin(0.05);
	////ANplot->SetRightMargin(0.05);
	//TH2D * temp2 = new TH2D("temp2","A_{N}",8,0.1,0.30,20,-0.1,0.15);
	////TH2D * temp = new TH2D("temp","A_{N}",8,0.1,0.35,20,-0.05,0.05);
