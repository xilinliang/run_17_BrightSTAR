...
void DiffJetXFANCalc_12np_7pt(TString inFileName, TString outName, TString outNameText, TString det)
{
	...
	for (int j=StartBin; j<nXFusebins; j=j+1)
	{
		...
		cout<<"AN result "<<j-StartBin<<": "<<ANresult[j-StartBin]<<"	AN statics un: "<<ANresultErr[j-StartBin]<<endl;
	...
	TH2D * temp = new TH2D("temp","A_{N}",8,0.1,0.45,30,-0.05,0.10);
...
//	T1.DrawLatex(0.11,-0.11,"STAR Preliminary");
	...
	TLatex T2;
   	...
   	T2.DrawLatex(0.11,-0.13,"p^{#uparrow} + p #rightarrow EM jet + p + X");
   	T2.DrawLatex(0.11,-0.15,"#sqrt{s} = 510 GeV");
   	T2.DrawLatex(0.11,-0.17,"3.0\% polarization scale uncertainty not shown");
	
	
	ANplot->SaveAs("FMS_ANplot_12np_7pt.pdf");
	ANplot->SaveAs("FMS_ANplot_12np_7pt.png");
	ANplot->SaveAs("FMS_ANplot_12np_7pt.root");
