...
//    TH1D *h1_PartNoRpPrPhi = new TH1D ("h1_PartNoRpPrPhi", "particle level proton Phi (not detected by RP)" + title + "; #phi [rad]", 50, -3.14, 3.14);
    ...
    TH1D *h1PartPrXL_West = new TH1D ("h1PartPrXL_West", "Particle level proton x_{L} sorted by west side" + title + "; x_{L}", 50, 0.0, 1.0);
    TH1D *h1PartPrXL_West_RPcoverage = new TH1D ("h1PartPrXL_West_RPcoverage", "Particle level proton x_{L} sorted by west side (RP coverage)" + title + "; x_{L}", 50, 0.0, 1.0);
    TH1D *h1PartPrXL_West_RPcoverage_EMjet = new TH1D ("h1PartPrXL_West_RPcoverage_EMjet", "Particle level proton x_{L} sorted by west side (within RP coverage and with EM-jet)" + title + "; x_{L}", 50, 0.0, 1.0);
...
//				if (RpEastTrkP >0) h2_RPPrP_vs_PartPrP_east->Fill(RpEastTrkP,genpart->momentum().P());
			...
			else
			{
				...
				h1PartPrXL_West->Fill(1.0*(genpart->momentum().P()) / 255.);
				GetPr = true;
				if (!(check_hard_diff_event[evt] == 1)) continue;
				if ((genpart->momentum().Eta() > 5) && (genpart->momentum().Eta()< 7)) {h1PartPrXL_West_RPcoverage->Fill(1.0*(genpart->momentum().P()) / 255.);}
				sumE = genpart->momentum().P();
				WestRP_P = genpart->momentum().P();
				continue;
//				if (RpWestTrkP >0) h2_RPPrP_vs_PartPrP_west->Fill(RpWestTrkP,genpart->momentum().P());
		...
		if ((genpart->GetStatus() > 0)&& ((genpart->GetFirstDaughter()) == 0))
		{
			...
			if ((genpart->GetId() == 22)&&(eta > 2.5)&&(eta<4.4)) 
			{
...
*/
	//cout << WestRP_P << endl;
	if ((EMjet)&&(WestRP_P > 0)) {h1PartPrXL_West_RPcoverage_EMjet->Fill(1.0*WestRP_P / 255.);}
}
    outFile->Write();
    outFile->Close();
    //muDstMaker = 0;
