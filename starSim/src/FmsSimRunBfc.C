...
void FmsSimRunBfc(int nevents, const char* fzfile, int filterThreshold = 1) 
{
    ...
    //const char* chainopt = "ry2017a agml usexgeom MakeEvent McEvent vfmce Idst BAna l0 l3 Tree logger fmsSim fmspoint evout -dstout IdTruth bigbig fzin geantout clearmem sdt20150417.193427"; //MMM's jobs
	       const char* chainopt = "DbV20220729 y2017 MakeEvent ITTF NoSsdIt NoSvtIt Idst BAna l0 Tree logger Sti VFPPVnoCTB beamLine tpcDB TpcHitMover TpxClu fmsDb fmsSim fmspoint  tags emcY2 EEfs eess evout -dstout IdTruth geantout big fzin MiniMcMk clearmem "; //Mriganka
...
{
    FmsSimRunBfc(-1, Form("FmsSim_Run17_%i_evt%i.fzd", cycle, nevents), filterThreshold);     
}
