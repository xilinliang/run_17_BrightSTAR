...
void RunFmsJetFinderPro(TString inMuDstFile, TString outJetName, Int_t nEvents)
{
    ...
    starDb->SetDateTime(20220729,191503);
    ...
    anapars12->addTpcCut(new StjTrackCutPt(0.2, 510));
    ...
    anapars12->addFmsCut(new StjTowerEnergyCutFMS(1.0, 255)); //min, max //Latif: Photon energy cut allows to reject low energy photons / hadronic contribitions and match a lot better with data
    //* 3 GeV cut was determined by RUN15 calibration condition: Zgg < 0.7 + pairE > 20 GeV

    //Jet cuts
    anapars12->addJetCut(new StProtoJetCutPt(0.01, 255));
    ...
    anaparsPart->addJetCut(new StProtoJetCutPt(0.01, 255));
...
{
    TString inMuDstFile = Form("FmsSim_Run17_%i_evt%i.MuDst.root", cycle, nEntries);
    TString outJetFile = Form("FmsJet_Run17_%i_evt%i.root", cycle, nEntries);
