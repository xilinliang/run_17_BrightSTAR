...
void RunFmsJetMakerSim(Int_t nEvents = -1, const char* inFile = "pythiaOut.MuDst.root", const char* pythiaFile = "pythiaOut.pythia.root", const char* outFile = "fmsJet.root")
{
    ...
    dbMk->SetDateTime(20220729,193427);
    ...
    anapars12->addJetCut(new StProtoJetCutPt(.01,510));
    ...
    anaparsParticle->addJetCut(new StProtoJetCutPt(1,510));
    ...
    anaparsParticleAll->addJetCut(new StProtoJetCutPt(1,510));
    ...
    // Jet cuts
    anaparsParton->addJetCut(new StProtoJetCutPt(3,510));
